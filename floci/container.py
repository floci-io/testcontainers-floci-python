...
class FlociContainer(DockerContainer):
    ...
    def __init__(self, image: str = f"{DEFAULT_IMAGE}:{DEFAULT_TAG}", **kwargs: object) -> None:
        ...
        self._dedicated_network: Optional[str] = None
    ...
    def get_dedicated_network_name(self) -> Optional[str]:
    ...
    def with_dedicated_network(self) -> "FlociContainer":
        ...
        import uuid
        name = f"floci-{uuid.uuid4().hex[:8]}"
        self._dedicated_network = name
        self.with_env("FLOCI_SERVICES_DOCKER_NETWORK", name)
        return self
    ...
    def with_resource_groups_tagging_config(self, config: ResourceGroupsTaggingConfig) -> "FlociContainer":
        config.apply_to(self)
        return self
